#!/usr/bin/python3
"""Define unittests for models/city.py."""
import os
import models
import unittest
from datetime import datetime
import time
from models.city import City


class TestCity_instants(unittest.TestCase):
    """Unittests for testing instantiation of the City class."""

    def test_no_args(self):
        self.assertEqual(City, type(City()))

    def test_new_instance(self):
        self.assertIn(City(), models.storage.all().values())

    def test_id(self):
        self.assertEqual(str, type(City().id))

    def test_created_at(self):
        self.assertEqual(datetime, type(City().created_at))

    def test_updated_at(self):
        self.assertEqual(datetime, type(City().updated_at))

    def test_state_id(self):
        cty = City()
        self.assertEqual(str, type(City.state_id))
        self.assertIn("state_id", dir(cty))
        self.assertNotIn("state_id", cty.__dict__)

    def test_name_is_public_class_attribe(self):
        cty = City()
        self.assertEqual(str, type(City.name))
        self.assertIn("name", dir(cty))
        self.assertNotIn("name", cty.__dict__)

    def test_two_cities_with_unique_ids(self):
        cty1 = City()
        cty2 = City()
        self.assertNotEqual(cty1.id, cty2.id)

    def test_two_cities_with_different_created_at(self):
        cty1 = City()
        time.sleep(0.05)
        cty2 = City()
        self.assertLess(cty1.created_at, cty2.created_at)

    def test_two_cities_with_different_updated_at(self):
        cty1 = City()
        time.sleep(0.05)
        cty2 = City()
        self.assertLess(cty1.updated_at, cty2.updated_at)

    def test_str_repr(self):
        dte = datetime.today()
        dte_repr = repr(dte)
        cty = City()
        cty.id = "456789"
        cty.created_at = cty.updated_at = dte
        cty_str = cty.__str__()
        self.assertIn("[City] (456789)", cty_str)
        self.assertIn("'id': '456789'", cty_str)
        self.assertIn("'created_at': " + dte_repr, cty_str)
        self.assertIn("'updated_at': " + dte_repr, cty_str)

    def test_unused_args(self):
        cty = City(None)
        self.assertNotIn(None, cty.__dict__.values())

    def test_instants_with_kwargs(self):
        dte = datetime.today()
        dte_iso = dte.isoformat()
        cty = City(id="123", created_at=dte_iso, updated_at=dte_iso)
        self.assertEqual(cty.id, "123")
        self.assertEqual(cty.created_at, dte)
        self.assertEqual(cty.updated_at, dte)

    def test_instants_without_kwargs(self):
        with self.assertRaises(TypeError):
            City(id=None, created_at=None, updated_at=None)


class TestCity_save(unittest.TestCase):
    """Unittests for testing save method."""

    @classmethod
    def setUp(self):
        try:
            os.rename("file.json", "x")
        except IOError:
            pass

    def tearDown(self):
        try:
            os.remove("file.json")
        except IOError:
            pass
        try:
            os.rename("x", "file.json")
        except IOError:
            pass

    def test_one_save(self):
        cty = City()
        time.sleep(0.05)
        first_updated_at = cty.updated_at
        cty.save()
        self.assertLess(first_updated_at, cty.updated_at)

    def test_two_saves(self):
        cty = City()
        time.sleep(0.05)
        first_updated_at = cty.updated_at
        cty.save()
        second_updated_at = cty.updated_at
        self.assertLess(first_updated_at, second_updated_at)
        time.sleep(0.05)
        cty.save()
        self.assertLess(second_updated_at, cty.updated_at)

    def test_save_with_args(self):
        cty = City()
        with self.assertRaises(TypeError):
            cty.save(None)

    def test_save_updates(self):
        cty = City()
        cty.save()
        cty_id = "City." + cty.id
        with open("file.json", "r") as file:
            self.assertIn(cty_id, file.read())


class TestCity_to_dict(unittest.TestCase):
    """Unittests for testing to_dict method of the City class."""

    def test_to_dict_type(self):
        self.assertTrue(dict, type(City().to_dict()))

    def test_to_dict_with_keys(self):
        cty = City()
        self.assertIn("id", cty.to_dict())
        self.assertIn("created_at", cty.to_dict())
        self.assertIn("updated_at", cty.to_dict())
        self.assertIn("__class__", cty.to_dict())

    def test_to_dict_has_added_attributes(self):
        cty = City()
        cty.middle_name = "ALX"
        cty.my_number = 89
        self.assertEqual("ALX", cty.middle_name)
        self.assertIn("my_number", cty.to_dict())

    def test_to_dict_datetime_attributes(self):
        cty = City()
        cy_dict = cty.to_dict()
        self.assertEqual(str, type(cy_dict["id"]))
        self.assertEqual(str, type(cy_dict["created_at"]))
        self.assertEqual(str, type(cy_dict["updated_at"]))

    def test_to_dict_output(self):
        dte = datetime.today()
        cty = City()
        cty.id = "456789"
        cty.created_at = cty.updated_at = dte
        my_dict = {
            'id': '456789',
            '__class__': 'City',
            'created_at': dte.isoformat(),
            'updated_at': dte.isoformat(),
        }
        self.assertDictEqual(cty.to_dict(), my_dict)

    def test_to_dict_with_args(self):
        cty = City()
        with self.assertRaises(TypeError):
            cty.to_dict(None)


if __name__ == "__main__":
    unittest.main()
